"""Build V2 company-year features with period-aware INSEE identity.

V2 vs V1 difference is one semantic change — the identity join now uses
...
but we still rewrite them under `risk_labels_v2/` so the V2 run is fully
self-contained and easy to audit against V1.
"""
...
logger = logging.getLogger("build_company_year_features_v2")
...
def build_company_year_datasets(
    *,
    ...
    company_year_dir = features_dir / "company_year_features_v2"
    labels_dir = features_dir / "risk_labels_v2"
    company_features_dir = features_dir / "company_features_v2"
    ...
    try:
        ...
        _write_manifest(
            company_year_dir,
            {
                "dataset": "company_year_features_v2",
                "rows": feature_rows,
        ...
        _write_manifest(
            ...
            {
                "dataset": "company_features_v2",
                "rows": _count(con, "company_feature_rows"),
                "source": "latest prediction year from company_year_features_v2",
            },
        )
    finally:
        con.close()
...
class _SourceRoots:
    def __init__(self, data_lake_dir: Path) -> None:
        ...
        # V2: strict — the period-aware identity table is the *only* accepted
        # source. No fallback to the V1 snapshot or to raw INSEE, because the
        # whole point of V2 is the temporal correctness of this join.
        self.company_identity = _first_dataset(
            data_lake_dir / "clean" / "company_identity_periodic",
        )
        if self.company_identity is None:
            raise RuntimeError(
                "V2 requires data-lake/clean/company_identity_periodic/. "
                "Run Phase 1 first (app.tools.v2.build_company_identity_periodic)."
            )
...
def _create_company_identity_view(con: Any, root: Path | None) -> None:
    # V2 schema: matches columns produced by
    # app.tools.v2.build_company_identity_periodic. Both period_start and
    # period_end are guaranteed non-null in the source. `closure_date` and
...
def _create_feature_tables(con: Any) -> None:
    ...
    con.execute(
        ...
        WITH identity_features AS (
            -- V2 identity join: strict interval match against the period-aware
            -- INSEE table built in Phase 1. period_end is exclusive (latest
            -- open period = 9999-12-31). Periods are guaranteed non-overlapping
            -- by construction, so at most ONE row matches per
            -- (siren, prediction_date) -- no aggregation needed.
            SELECT
                b.siren,
                b.prediction_year,
                b.prediction_date,
            ...
            LEFT JOIN company_identity i
             ...
             AND i.period_end   >  b.prediction_date
        ),
        legal_features AS (
            SELECT
                b.siren,
    ...
    con.execute(
        ...
        future_insee AS (
            SELECT
                b.siren,
                b.prediction_year,
                -- V2: closure_date isn't in the periodic identity table; we
                -- detect a future cessation via a future period whose
                -- administrative_status flips to 'cessé/fermé/inactif'.
                max(
                    CASE WHEN
